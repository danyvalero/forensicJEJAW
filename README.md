# forensicJEJAW
    ...
    Developed by Wilson Barios, Jorge Ortiz, Edgar Robayo, Jose Torregroza y Daniel Valero's Team Forensic JEJAW
...
Nota.
Para cada ejecución de programa, este guarda la información en un archivo de texto capturando el programa ejecutado y la hora local del equipo, este resultado se verá en la carpeta Result y será guardado en un archivo.txt


1.  ANÁLISIS DE MEMORIRA RAM

Aquí puede encontrar los programas volátiles, Para obtener más información sobre Sysinternals y estas utilidades, visite el sitio web - http://technet.microsoft.com/sysinternals
...
AdExplorer - Active Directory Explorer es un visor y editor avanzado de Active Directory (AD).
...
Aquí puede encontrar programas para generar calcular hashes MD5, SHA-1 y demás a uno o más archivos del sistema.

Para ejecutar los programas debe ejecutar la siguiente instrucción: nombre_programa.exe , si requiere ayuda sobre la ejecución de ellos puede escribir con la bandera -h donde muestra una ayuda del programa, por ejemplo nombre_programa.exe -h ; si el programa requiere alguna bandera deberá consultar la ayuda del programa y luego su respectivo comando, por ejemplo nombre_programa.exe -l

HashMyFiles.exe - es una pequeña utilidad que permite calcular los hashes MD5 y SHA1

md5summer.exe - MD5summer es una aplicación para Microsoft Windows 9x, NT, ME, 2000 y XP que genera y verifica sumas de comprobación md5. Su archivo de salida es compatible con la salida del GNU MD5Sum de Linux y también leerá los archivos generados por Linux.


3. FORENSE

Aquí puede encontrar programas para realizar un análisis forense, montar/desmontar imágenes forenses y demás.

Para ejecutar los programas debe ejecutar la siguiente instrucción: nombre_programa.exe , si requiere ayuda sobre la ejecución de ellos puede escribir con la bandera -h donde muestra una ayuda del programa, por ejemplo nombre_programa.exe -h ; si el programa requiere alguna bandera deberá consultar la ayuda del programa y luego su respectivo comando, por ejemplo nombre_programa.exe -l

adencrypt_gui.exe - Es una encriptación de imagen completa. No sólo se encriptan los datos en bruto (por ejemplo, los datos del sector)
sino también los metadatos de la imagen. Cada segmento (archivo) de la imagen se con una clave de imagen generada aleatoriamente mediante un cifrado potente (AES-256).

DiskDigger.exe - Es una herramienta que deshace y recupera los archivos perdidos de su disco duro, tarjetas de memoria, unidades flash USB... ¡lo que sea! Tanto si has borrado accidentalmente algunos documentos o fotos de tu ordenador, como si has reformateado la tarjeta de memoria de tu cámara o quieres ver qué archivos se esconden en una vieja unidad USB, DiskDigger.

EMFSpoolViewer.exe - Este programa le ayudará a ver lo que usted u otra persona imprime en su impresora
...
TimeStampClientGui.exe - Aplicación GUI utilizable en Windows
